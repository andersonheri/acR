---
title: "Análise de Conteúdo Qualitativa com LLMs via ellmer"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Análise de Conteúdo Qualitativa com LLMs via ellmer}
...
knitr::opts_chunk$set(
  collapse  = TRUE,
  comment   = "#>",
  eval      = FALSE   # chunks não são avaliados no build; remova para execução local
)
```

## Visão Geral

O pacote **acR** oferece duas funções para análise de conteúdo qualitativa assistida por
modelos de linguagem de grande escala (LLMs):

| Função              | O que faz                                                        |
|---------------------|------------------------------------------------------------------|
| `ac_qual_code()`    | Classifica textos em categorias de um *codebook* existente       |
| `ac_qual_codebook()`| Induz um *codebook* a partir de um corpus de textos              |

A partir da versão 0.3.0, ambas as funções aceitam o argumento `chat =`, que recebe
qualquer objeto `Chat` do pacote **ellmer**. Isso permite usar **qualquer provedor
suportado pelo ellmer** — OpenAI, Google Gemini, Groq, Anthropic, Ollama, Mistral,
DeepSeek, entre outros — sem alterar a lógica de análise.

> **Retrocompatibilidade:** quando `chat = NULL` (padrão), as funções usam o
> comportamento anterior baseado em `provider` e `api_key`, mantendo compatibilidade
> total com código existente.
...
O `ellmer` padroniza a interface com qualquer LLM em um objeto `Chat` criado pelas
funções `chat_*()`. O acR usa esse objeto diretamente, delegando ao ellmer toda a
...
1. **Configurar** a chave de API via variável de ambiente (`.Renviron`).
2. **Instanciar** o objeto `Chat` com `ellmer::chat_*()`.
3. **Passar** o objeto para `ac_qual_code()` ou `ac_qual_codebook()` via `chat =`.

---

## Dados de exemplo

Para reproduzir os exemplos abaixo, usamos um corpus de discursos parlamentares
sobre política de saúde e um *codebook* temático simples.
...
`gemini-2.5-flash` é rápido e preciso para classificação de textos em português.
...
O Groq é especialmente útil para prototipagem rápida: latência muito baixa e
plano gratuito generoso para modelos como `llama-3.3-70b-versatile`.
...
Ideal quando os textos contêm informações sensíveis (entrevistas, documentos
internos de gestão pública) e não podem ser enviados a APIs externas.
Requer [Ollama](https://ollama.com) instalado localmente.
...
O OpenRouter permite acessar modelos de múltiplos provedores (Anthropic, Google,
Meta, Mistral etc.) com uma única chave de API, útil para comparar resultados.
...
modificações. Quando `chat = NULL`, as funções usam os argumentos legados
`provider` e `api_key`:
...
> **Recomendação:** migre gradualmente para `chat =` em projetos novos, pois esse
> padrão oferece controle mais fino sobre o modelo, o `system_prompt` e os parâmetros
> de geração.
...
Um uso avançado é comparar a concordância entre diferentes modelos no mesmo corpus,
uma estratégia análoga à avaliação de confiabilidade intercodificadores
(Krippendorff, 2004).
...
O `system_prompt` é o principal vetor de customização do comportamento do modelo.
Recomendações para análise de conteúdo:
...
O método recomendado é armazenar todas as chaves no arquivo `.Renviron` do usuário,
editável com `usethis::edit_r_environ()`:
...
Depois de salvar, reinicie o R. Verifique com `Sys.getenv("GOOGLE_API_KEY")`.

Nunca inclua chaves diretamente no código-fonte ou em arquivos versionados.
...
| OpenAI           | `chat_openai()`           | `OPENAI_API_KEY`       | ❌ Pago       |
| Anthropic Claude | `chat_anthropic()`        | `ANTHROPIC_API_KEY`    | ❌ Pago       |
| Mistral          | `chat_mistral()`          | `MISTRAL_API_KEY`      | ❌ Pago       |
| DeepSeek         | `chat_deepseek()`         | `DEEPSEEK_API_KEY`     | ⚠️ Limitado  |
| OpenRouter       | `chat_openrouter()`       | `OPENROUTER_API_KEY`   | ⚠️ Por uso   |

---

## Referências

KRIPPENDORFF, K. **Content Analysis: An Introduction to Its Methodology**. 3. ed.
Thousand Oaks: SAGE, 2013.

NEUENDORF, K. A. **The Content Analysis Guidebook**. 2. ed. Thousand Oaks: SAGE, 2017.

WICKHAM, H. et al. **ellmer: Chat with Large Language Models**. R package version 0.2.0.
Disponível em: <https://ellmer.tidyverse.org>. Acesso em: abr. 2026.

BAIL, C. A. Breaking the social media prism: How to make our platforms less polarizing.
**American Journal of Sociology**, v. 127, n. 2, p. 661-663, 2021.
