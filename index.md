...
instruções de classificação. Pode ser criado manualmente ou carregado de
um arquivo YAML para reuso entre projetos.
...
parâmetro `k_consistency = 3` ativa o cálculo de certeza via
*self-consistency* (Wang et al., 2023): o modelo classifica cada
documento três vezes e a confiança é a proporção de concordância entre
as rodadas.
...
consulta a API do [OpenAlex](https://openalex.org/) (gratuita, sem
chave) para recuperar referências reais com DOI, abstract e número de
citações verificados, usando a LLM apenas para sintetizar o abstract em
português. Isso evita alucinações bibliográficas, um problema comum
quando se pede a um LLM que “invente” referências sem consultar uma
fonte externa.
